...
export default function MyUsageTab() {
  const { tabData, lang, theme, refreshTab } = useApp();
  const myUseData = tabData.myuse;
  ...
  useEffect(() => {
    if (myUseData?.savings?.net_savings !== undefined) {
      const target = myUseData.savings.net_savings;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-left animate-tab-in">
        <h1 className="text-2xl font-black tracking-tight">{t('nav_myuse', lang) || 'Thống kê tiêu dùng'}</h1>
        <p className="text-xs text-base-content/60 mt-1">{t('myuse_sub', lang) || 'Theo dõi chi tiết số lượng Token đã sử dụng và lượng tiền tiết kiệm được của riêng bạn'}</p>
      </div>
