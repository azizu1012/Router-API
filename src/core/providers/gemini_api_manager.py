...
class GeminiAPIManager(GeminiAPIHelpersMixin):
    ...
    async def call_gemini_stream(
        ...
        image_tokens = image_count * 258
        logger.info("call_gemini_stream checking quota for model %s", model_alias)
        has_quota = await self._acquire_gemini_quota(prompt_text, model_alias, image_tokens=image_tokens)
        logger.info("call_gemini_stream quota check result: %s", has_quota)
        if not has_quota:
            raise RuntimeError("quota_exhausted")
        ...
        for attempt in range(1, config.MAX_RETRIES + 1):
            logger.info("call_gemini_stream attempt %d/%d starting", attempt, config.MAX_RETRIES)
            now = time.time()
            if now < router.global_cooldown_until:
                wait = router.global_cooldown_until - now + 1.0
                logger.info("Global cooldown active, waiting %.1fs (attempt %d/%d)", wait, attempt, config.MAX_RETRIES)
            ...
            logger.info("call_gemini_stream starting key scanning for total_keys=%d", total_keys)
            for _ in range(total_keys):
                if _ > 0:
                    await asyncio.sleep(0.3 + random.uniform(0, 0.3))
                ...
                logger.info("call_gemini_stream calling _get_best_api_key with exclude_models=%s", exclude_models)
                api_key, model_id, _, reservation = await self._get_best_api_key(model_alias, account=account, estimated_tokens=estimated_total_tokens, exclude_models=exclude_models)
                logger.info("call_gemini_stream _get_best_api_key returned api_key=%s", api_key[-4:] if api_key else None)
                if not api_key:
                    break
                ...
                try:
                    ...
                    client = await self._get_client(api_key)
                    logger.info("Gemini stream call attempt %d/%d model=%s key=...%s", attempt, config.MAX_RETRIES, model_alias, api_key[-4:])
                    ...
                    try:
                        stream_response_generator = self._generate_gemini_content_stream(
                            api_key=api_key, model_id=model_id,
                            system_instruction=system_instruction, contents=contents,
                            max_tokens=max_tokens, temperature=temperature,
                            top_p=top_p, tools=request_tools or None,
                        ...
                        full_response_parts = []
                        async for chunk_response in stream_response_generator:
                            chunk_dict = chunk_response.model_dump(by_alias=True, exclude_none=True)
                            yield {
                            ...
                            # Accumulate parts for usage logging (only text/function_call)
                                    ...
                                    if candidate.content and candidate.content.parts:
                                        for part in candidate.content.parts:
                                            if part.text:
                                                ...
                                                full_response_parts.append(f"function_call:{part.function_call.name}")

                        # After streaming is complete, log usage
                        usage = getattr(chunk_response, "usage_metadata", None) # Last chunk should have usage
                        input_tokens = getattr(usage, "prompt_token_count", 0) or 0
                        output_tokens = getattr(usage, "candidates_token_count", 0) or 0
                        
                        # Fallback for output tokens if usage_metadata is missing from chunks (older SDKs)
                        if output_tokens == 0 and full_response_parts:
                            # Simple estimation, will be improved with real tokenization later
                            output_tokens = len("".join(full_response_parts)) // 4 
                        ...
                        if use_grounding and is_grounding_error:
                            ...
                            # Re-yield chunks from the retried stream
                            full_response_parts = []
                            async for chunk_response in stream_response_generator:
                                chunk_dict = chunk_response.model_dump(by_alias=True, exclude_none=True)
                                yield {
                            ...
                            usage = getattr(chunk_response, "usage_metadata", None)
                            input_tokens = getattr(usage, "prompt_token_count", 0) or 0
                            output_tokens = getattr(usage, "candidates_token_count", 0) or 0
                            if output_tokens == 0 and full_response_parts:
                                output_tokens = len("".join(full_response_parts)) // 4
