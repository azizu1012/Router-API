...
class ModelPool:
    ...
    def __init__(self, pool_config: dict):
        ...
        self.max_attempts = int(pool_config["max_attempts"])
        self.current_idx = 0
    ...
    def effective_swap_threshold(self, reason: str = "") -> int:
        if reason in self.HARD_FAILURE_REASONS:
            return 1
        if reason in self.TRANSIENT_FAILURE_REASONS:
            return min(self.swap_failures, 2)
    ...
    def swap(self) -> bool:
        if self.total_attempts >= self.max_attempts:
            return False
        # If all members are currently exhausted but we still have attempts left,
        # reset the exhaustion state so we can cycle through them again.
    ...
    def exhausted(self) -> bool:
        if self.total_attempts >= self.max_attempts:
            return True
        # If all members are exhausted but we have attempts remaining,
        # we are not exhausted yet (swap() will reset the exhaustion states)
        if len(self._exhausted_models) >= len(self.members):
            return False
        return False
