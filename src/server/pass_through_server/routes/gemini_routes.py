...
async def _stream_gemini_native(resp_dict: Dict[str, Any]) -> AsyncIterator[bytes]:
    ...
    template_dict = copy.deepcopy(resp_dict)
    chunk_size = 900
    for offset in range(0, len(full_text), chunk_size):
        is_last = (offset + chunk_size) >= len(full_text)
        chunk_text = full_text[offset:offset + chunk_size]
        
        chunk = copy.deepcopy(template_dict)
        if "candidates" in chunk and chunk["candidates"]:
            cand = chunk["candidates"][0]
            if "content" in cand and "parts" in cand["content"] and cand["content"]["parts"]:
                cand["content"]["parts"][0]["text"] = chunk_text
            if not is_last:
                cand.pop("finishReason", None)
...
async def _handle_gemini_native(
    ...
    await log_usage(
        gresult.get("model_alias") or model_alias,
    ...
    if stream:
        return StreamingResponse(_stream_gemini_native(resp_dict), media_type="text/event-stream")
