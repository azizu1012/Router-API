...
22. **Thinking Adaptive Normalization (v2.4.2)**: `type: "adaptive"` maps to `{"thinking": {"type": "enabled", "budget_tokens": 4096}}` for flash, `8192` for pro. Enough thinking for quality but capped to avoid indefinite think time. The `display: "summarized"` field from Claude Code is stripped since it's invalid for Gemini/litellm. Dict copy is avoided — a new dict is constructed with only valid fields.
